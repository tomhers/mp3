...
@str.0 = internal constant [10 x i8] c"plus_o.cl\00", align 1
...
ok.1:
	...
	%tmp.6 = load %Main_vtable*, %Main_vtable** %tmp.5
	%tmp.7 = getelementptr %Main_vtable, %Main_vtable* %tmp.6, i32 0, i32 11
	%tmp.8 = load i32 (%Main*) *, i32 (%Main*) ** %tmp.7
	%tmp.9 = call i32(%Main*) %tmp.8( %Main* %tmp.3 )
	%tmp.10 = load %Main*, %Main** %tmp.2
...
ok.0:
	...
	%tmp.17 = load %Main*, %Main** %tmp.2
	%tmp.18 = bitcast %Main* %tmp.17 to %Object*
	ret %Object* %tmp.18
...
entry:
	%tmp.19 = getelementptr %Main_vtable, %Main_vtable* @Main_vtable_prototype, i32 0, i32 1
	...
	store %Main_vtable* @Main_vtable_prototype, %Main_vtable** %tmp.23
	%tmp.24 = alloca %Main*
	store %Main* %tmp.22, %Main** %tmp.24
	ret %Main* %tmp.22
